    ...
    <div v-if="!isLoading">
        ...
        <section v-if="data.items && data.items.length">
            ...
            <div class="list" v-if="data.items && data.items.length">
                <router-link :to="{name: 'routeEdit', params: {id: item.id}}" :key="i" v-for="(item, i) in data.items" @click.native="setHidePreserve">
                    <div class="item">
...
<style scoped src="../css/SalesByRoute.css"/>
<script>
import Header from '../../components/Header'
import lang from '../../translations'
import Footer from '../../components/Footer'
import {imagesPathRewrite, routeSalesPerPage} from '../../config'
...
export default {
    ...
    methods: {
        showMore() {
            ...
            this.$store.dispatch('apiCall', {
                ...
                data: {lang: this.$store.state.locale, skip: this.skip, mobile: true, history: true}
            }).then(data => {
        ...
        setHidePreserve() {
            this.$store.commit('setRouteEditHidePreserve', 1)
            this.$store.commit('setRouteEditSmallHeader', 1)
    ...
    mounted() {
        ...
        this.$store.dispatch('apiCall', {actionName: 'getSalesByRoute', data: {lang: this.$store.state.locale, skip: this.skip, mobile: true, history: true}}).then(data => {
            this.data = data.data
